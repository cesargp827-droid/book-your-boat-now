...
const boats = [
  {
    ...
    pack: { perPerson: 35, totalBase: 245, duration: "4h (+2h gratis)" },
  },
  {
    value: "nireus-620",
    name: "Nireus 620 CL",
    ...
    pack: { perPerson: 32, totalBase: 320, duration: "4h (+2h gratis)" },
  },
  {
    value: "astilux-600",
    ...
    pack: { perPerson: 40, totalBase: 280, duration: "4h (+2h gratis)" },
  },
];
...
const BookingPage = () => {
  ...
  const durationParam = searchParams.get("duration") || "4h";
  const skipperParam = searchParams.get("skipper") === "1";

  const isPack = !!packParam;

  const [step, setStep] = useState(1);
  const [date, setDate] = useState<Date | undefined>();
  const [duration] = useState(isPack ? "4h" : durationParam as "4h" | "8h");
  ...
  const [offerType, setOfferType] = useState<"basic" | "pack">(isPack ? "pack" : "basic");

  const selectedBoat = useMemo(() => boats.find((b) => b.value === boatParam), [boatParam]);

  const basePrice = useMemo(() => {
    if (!selectedBoat) return 0;
    if (offerType === "pack") return selectedBoat.pack.totalBase;
    return selectedBoat.prices[duration as keyof typeof selectedBoat.prices] || 0;
  }, [selectedBoat, offerType, duration]);
    ...
    return (
      <div className="min-h-screen bg-secondary flex items-center justify-center">
        <div className="text-center">
          <p className="font-body text-muted-foreground mb-4">No se ha seleccionado ninguna embarcación.</p>
          <Button onClick={() => navigate("/")} variant="outline">Volver a la flota</Button>
        </div>
      </div>
    );
  }
      ...
      <div className="container mx-auto px-4 py-8 lg:py-12">
        <div className="max-w-5xl mx-auto grid lg:grid-cols-[1fr_380px] gap-8">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-card p-8 lg:p-10">
            ...
            {step === 1 && (
              <div>
                <h2 className="font-display text-2xl font-bold text-foreground mb-6">Selecciona la fecha</h2>
            ...
            {step === 2 && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="font-display text-2xl font-bold text-foreground">Selecciona el horario</h2>
                  <button onClick={goBack} className="font-body text-sm text-primary hover:underline">Cambiar fecha</button>
                </div>

                <p className="font-body text-sm font-semibold text-foreground mb-3">Duración:</p>
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className={cn(
                    "rounded-xl px-5 py-4 font-body text-sm font-semibold text-center border-2",
                    "bg-primary text-primary-foreground border-primary"
                  )}>
                    {offerType === "pack"
                      ? `4 horas (+2h gratis)`
                      : duration === "4h" ? "4 horas" : "8 horas (Día completo)"}
                  </div>
                </div>
                  ...
                  {currentTimeSlots.map((slot) => (
                    <button
                      ...
                      className={cn(
                        ...
                        timeSlot === slot.value ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/40"
                      )}
                    >
                      <p className="font-body text-sm font-semibold text-primary flex items-center gap-2">
                        <Clock className="h-4 w-4" /> {slot.label}
                      </p>
                      <p className="font-body text-xs text-muted-foreground mt-1">{basePrice}€</p>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {step < 4 && (
              <div className="flex items-center justify-between mt-10 pt-6 border-t border-border">
                {step > 1 ? (
                  <Button variant="ghost" onClick={goBack} className="font-body"><ArrowLeft className="h-4 w-4 mr-2" /> Atrás</Button>
                ) : <div />}
                {step === 3 ? (
          ...
          <div className="lg:sticky lg:top-8 self-start">
            <div className="bg-card rounded-2xl border border-border shadow-card overflow-hidden p-6">
              <h3 className="font-display text-xl font-bold text-foreground mb-5">Resumen de reserva</h3>
              ...
              {/* Pack toggle */}
              <div className="mb-5">
                <p className="font-body text-sm font-semibold text-foreground mb-2">Pack seleccionado:</p>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => setOfferType("basic")}
                    className={cn(
                      "rounded-lg py-2.5 font-body text-sm font-semibold transition-all",
                      offerType === "basic" ? "bg-muted text-foreground" : "bg-card text-muted-foreground border border-border"
                    )}
                  >
                    Básico
                  </button>
                  <button
                    ...
                    className={cn(
                      ...
                      offerType === "pack" ? "bg-gold text-gold-foreground" : "bg-card text-muted-foreground border border-border"
                    )}
                  >
                    VIP
                  </button>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-body font-semibold text-primary">
                    {offerType === "pack" ? selectedBoat.pack.duration : (duration === "4h" ? "4 horas" : "8 horas")}
                  </span>
                </div>
              </div>

              {/* Skipper toggle in sidebar */}
              <div className="flex items-center justify-between mt-5 bg-secondary rounded-xl p-3">
                <div>
                  <p className="font-body text-xs font-semibold text-foreground">Patrón</p>
                ...
                <div className="text-right">
                  ...
                  {offerType === "pack" && (
                    <p className="font-body text-xs text-muted-foreground">
                      {Math.round(finalPrice / selectedBoat.capacity)}€ por persona
                    </p>
                  )}
                </div>
              </div>
